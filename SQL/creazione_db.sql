...
create table ospite (
    email varchar(320) not null unique, -- 64 characters for local part + @ + 255 for domain name
...
create type stato as enum ('accetata', 'rifiutata', 'in corso');
...
create table recensione (
    email_ospite varchar(320) not null,
    uuid_prenotazione uuid not null unique,
...
create table recensione_ospite (
    ...
    constraint RECENSIONE_OSPITE_DATA_RECENSIONE_FK_OSPITE foreign key(data_recensione_ospite) references recensione(data_recensione),
    constraint RECENSIONE_OSPITE_FK_RECENSIONE foreign key(uuid_prenotazione) references recensione(uuid_prenotazione)
);
...
create table recensione_host (
    ...
    constraint RECENSIONE_HOST_DATA_RECENSIONE_FK_OSPITE foreign key(data_recensione_host) references recensione(data_recensione),
    constraint RECENSIONE_HOST_FK_RECENSIONE foreign key(uuid_prenotazione) references recensione(uuid_prenotazione)
);
...
create type metodo_p as enum ('carata di credito/debito', 'Xpay');
...
create table composizione (
    ...
    data_creazione_lista_preferiti timestamp not null unique,
...
create table commento(
    ...
    data_commento timestamp not null,
    testo text not null,
